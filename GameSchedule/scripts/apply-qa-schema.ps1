...
$queries = @(
  ...
  @{ Name = "games-table"; Query = "create table if not exists public.games (id text primary key, title text not null unique, genre text not null, platform text not null, player_count text not null, description text, igdb_id bigint, cover_url text, release_date date, rating numeric, source text not null default 'seed', is_featured boolean not null default false, created_at timestamptz not null default now());" },
  @{ Name = "games-igdb-columns"; Query = "alter table public.games add column if not exists igdb_id bigint, add column if not exists cover_url text, add column if not exists release_date date, add column if not exists rating numeric, add column if not exists source text not null default 'seed';" },
  @{ Name = "games-igdb-index"; Query = "create unique index if not exists games_igdb_id_key on public.games (igdb_id) where igdb_id is not null;" },
  @{ Name = "games-source-check"; Query = "alter table public.games drop constraint if exists games_source_check; alter table public.games add constraint games_source_check check (source in ('seed', 'igdb'));" },
  ...
  @{ Name = "games-seed-1"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('helix-arena','Helix Arena','Hero Shooter','PC / Console','3-5 players','Fast team-based matches with hero abilities and short queue times.','seed',true) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "games-seed-2"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('drift-legends-x','Drift Legends X','Racing','PC / Console','2-8 players','Arcade racing nights with custom lobbies and private party queues.','seed',false) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "games-seed-3"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('deep-raid','Deep Raid','Extraction','PC','2-4 players','High-risk co-op missions with short planning sessions and long-term loot.','seed',true) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "games-seed-4"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('skyforge-party','Skyforge Party','MMO','Cross-platform','4-6 players','Dungeon runs and weekly guild goals for a repeat squad.','seed',false) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "games-seed-5"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('pixel-strike-zero','Pixel Strike Zero','Arena Shooter','PC','4-10 players','Retro-styled matches that work well for fast private lobby sessions.','seed',false) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "games-seed-6"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('wild-rally-online','Wild Rally Online','Racing','Cross-platform','2-12 players','Chaotic off-road playlists with short races and easy drop-in play.','seed',true) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "games-seed-7"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('void-divers','Void Divers','Co-op Action','PC / Console','2-4 players','Mission-based co-op with rotating objectives and squad builds.','seed',true) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "games-seed-8"; Query = "insert into public.games (id, title, genre, platform, player_count, description, source, is_featured) values ('castle-circuit','Castle Circuit','Party Strategy','PC / Mobile','2-6 players','Short tactical rounds with low setup cost and high rematch potential.','seed',false) on conflict (id) do update set title = excluded.title, genre = excluded.genre, platform = excluded.platform, player_count = excluded.player_count, description = excluded.description, source = excluded.source, is_featured = excluded.is_featured;" },
  @{ Name = "profile-games-table"; Query = "create table if not exists public.profile_games (profile_id uuid not null references public.profiles(id) on delete cascade, game_id text not null references public.games(id) on delete cascade, created_at timestamptz not null default now(), primary key (profile_id, game_id));" },
  ...
  @{ Name = "profile-games-rls"; Query = "alter table public.profile_games enable row level security;" },
  ...
  @{ Name = "profile-games-select-drop"; Query = "drop policy if exists ""Users can read their library games"" on public.profile_games;" },
  @{ Name = "profile-games-select"; Query = "create policy ""Users can read their library games"" on public.profile_games for select to authenticated using (auth.uid() = profile_id);" },
  @{ Name = "profile-games-insert-drop"; Query = "drop policy if exists ""Users can insert their library games"" on public.profile_games;" },
  @{ Name = "profile-games-insert"; Query = "create policy ""Users can insert their library games"" on public.profile_games for insert to authenticated with check (auth.uid() = profile_id);" },
  @{ Name = "profile-games-delete-drop"; Query = "drop policy if exists ""Users can delete their library games"" on public.profile_games;" },
  @{ Name = "profile-games-delete"; Query = "create policy ""Users can delete their library games"" on public.profile_games for delete to authenticated using (auth.uid() = profile_id);" },
  ...
  @{ Name = "profile-games-backfill-favorites"; Query = "insert into public.profile_games (profile_id, game_id) select favorite_games.profile_id, favorite_games.game_id from public.favorite_games on conflict (profile_id, game_id) do nothing;" },
  @{ Name = "profile-games-backfill-roulette"; Query = "insert into public.profile_games (profile_id, game_id) select roulette_pool_entries.profile_id, roulette_pool_entries.game_id from public.roulette_pool_entries on conflict (profile_id, game_id) do nothing;" },
  ...
  @{ Name = "profile-games-backfill-hosted-lobbies"; Query = "insert into public.profile_games (profile_id, game_id) select distinct lobbies.host_profile_id, lobbies.game_id from public.lobbies on conflict (profile_id, game_id) do nothing;" },
  @{ Name = "favorites-profile-games-fk-drop"; Query = "alter table public.favorite_games drop constraint if exists favorite_games_profile_game_membership_fkey;" },
  @{ Name = "favorites-profile-games-fk"; Query = "alter table public.favorite_games add constraint favorite_games_profile_game_membership_fkey foreign key (profile_id, game_id) references public.profile_games(profile_id, game_id) on delete cascade;" },
  @{ Name = "roulette-profile-games-fk-drop"; Query = "alter table public.roulette_pool_entries drop constraint if exists roulette_pool_entries_profile_game_membership_fkey;" },
  @{ Name = "roulette-profile-games-fk"; Query = "alter table public.roulette_pool_entries add constraint roulette_pool_entries_profile_game_membership_fkey foreign key (profile_id, game_id) references public.profile_games(profile_id, game_id) on delete cascade;" },
  @{ Name = "remove-game-from-library-function"; Query = 'create or replace function public.remove_game_from_library(p_game_id text) returns void language plpgsql security definer set search_path = public as $$ begin if auth.uid() is null then raise exception ''Authentication required''; end if; delete from public.favorite_games where profile_id = auth.uid() and game_id = p_game_id; delete from public.roulette_pool_entries where profile_id = auth.uid() and game_id = p_game_id; delete from public.profile_games where profile_id = auth.uid() and game_id = p_game_id; end; $$;' },
